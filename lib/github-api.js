...
// is the default, and any of the others can be mapped to a specific
// resource owner — a GitHub organization *or* a personal account login,
// GitHub draws no distinction between the two in a repo's `owner/repo`
// path, and neither does this mapping. Every call site that hits a
// repo-or-owner-scoped endpoint passes `{ owner }` so resolveTokenRecord()
// can pick the token mapped to that owner, falling back to the default
// when there's no mapping — never by trying every saved token in turn. A
// caller with no owner in scope at all (none currently) falls straight
// through to the default token.
  ...
  if (!migratePromise) {
    migratePromise = (async () => {
      ...
      await chrome.storage.local.set({
        githubTokens: [{ id, name: "Default", token: githubToken, owner: "" }],
        defaultTokenId: id,
      });
...
// token is removed — a rename or an owner re-mapping doesn't affect
...
// resources owned by `owner` (an organization or personal account
// login — GitHub itself doesn't distinguish the two in a resource's
// `owner/repo` path, so neither does this lookup): an explicit owner
// mapping wins over the default token. No further fallback — a mapped
// owner whose token is missing access should surface its own error
// rather than silently retrying under a different credential (per the
// "do not silently try every saved token" rule), and with no default
// configured an unmapped owner is requested without a token rather than
// with some other owner's.
export async function resolveTokenRecord(owner) {
  const records = await getTokenRecords();
  if (!records.length) return null;
  const normalizedOwner = (owner || "").trim().toLowerCase();
  if (normalizedOwner) {
    const mapped = records.find((r) => (r.owner || "").trim().toLowerCase() === normalizedOwner);
    if (mapped) return mapped;
  }
  const defaultId = await getDefaultTokenId();
  return records.find((r) => r.id === defaultId) || null;
}
