...
async function fetchAndInline(owner, repo, path, ref) {
  const html = decodeBase64Utf8(await fetchContentsBase64(owner, repo, path, ref));
  return inlineHtmlAssets(html, owner, repo, ref, dirOf(path));
}
...
async function resolveAndFetch(owner, repo, segments) {
  ...
  for (let k = 1; k < segments.length; k++) {
    ...
    try {
      return { html: await fetchAndInline(owner, repo, path, ref), ref, path };
    } catch (err) {
